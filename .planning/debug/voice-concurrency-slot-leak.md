...
## Resolution (BUG 1 REFINEMENT — terminal close releases IMMEDIATELY, not via the 12s grace)

trigger: "rev :13 live: reload worked 2x, the 3rd walled with concurrency-limit and stayed walled ~30s. The initial BUG 1 fix routed the raw connection `closed` event through `on_transport_disconnected()` -> the 12s reconnect grace. But a tab-close/RELOAD is TERMINAL: the returning client mints a brand-new session_id, so waiting 12s for a same-session reconnect that never comes is wrong. Two reloads inside the grace stack two draining slots and wall the 3rd at max_concurrent=2."

behavioral_finding: "VERIFIED against installed pipecat (transports/smallwebrtc/{connection,transport,request_handler}.py + utils/base_object.py): the `SmallWebRTCConnection` `closed` event is TERMINAL-ONLY. connection.py `_handle_new_connection_state` fires `_call_event_handler(state)` where state == pc.connectionState; `closed` fires only after aiortc `pc.close()` has run — reached via (a) graceful `disconnect()`, (b) an ICE `failed` self-close (`if state=='failed': await self._close()`), or (c) the 60s connecting-timeout. A TRANSIENT ICE blip fires the SEPARATE `disconnected` event, which transport.py routes to `_handle_peer_disconnected` (nulls tracks) and does NOT reach `on_client_disconnected`. A `restart_pc` renegotiation calls `pc.remove_all_listeners()` BEFORE `_close()`, so it never fires `closed` either. And after `closed` the request_handler pops the pc from `_pcs_map` while a returning client gets a fresh session_id from start_gate -> a same-session reconnect is IMPOSSIBLE. Conclusion: immediate release on `closed` is correct and safe; the reconnect grace protected a reconnect that can never happen on the terminal path. Chose IMMEDIATE release (not the ~2s-grace fallback) because `closed` is provably terminal-only."

fix: "server.py `_wire_connection_teardown`: the raw-connection `closed` handler now calls `lifecycle.release()` DIRECTLY (was `on_transport_disconnected()` -> 12s grace). The 12s D-07 reconnect grace is left UNCHANGED for the TRANSIENT path — the transport's `on_client_disconnected` -> `on_transport_disconnected` in `_run_session` — so mid-session ICE recovery via `on_transport_reconnected` still works. release() is the single idempotent teardown: its `_stopped` guard makes the double-fire (`closed` + a racing `on_client_disconnected`) a no-op and it cancels any pending reconnect-grace task, so immediate release always wins regardless of BaseObject's concurrent-task dispatch order. HARDENING: added an `if self._stopped: return` guard in `session.py` SessionLifecycle.start() just before the tick/timer/watchdog task creation — immediate release newly makes a 'released mid-start' interleave reachable (release() completing during start()'s `await asyncio.to_thread` calls); without the guard the tick loop would renew (re-lease) the very heartbeat release() just freed. release()'s task-cancel + this guard together cover both orderings (release-before-start-body and release-during-start)."

verification: "TDD: `test_abrupt_connection_close_releases_heartbeat_lease_immediately` — RED against rev :13 (grace=30s, no sleep -> count==1, slot held) then GREEN after the fix (release synchronous on `closed`: active_session_count==0, count_active_heartbeats==0, lease expiresAt<=now with NO grace sleep). New contrast lock `test_transport_disconnect_still_uses_the_reconnect_grace` GREEN — the TRANSIENT `on_transport_disconnected` path still holds the slot within the grace then releases after it elapses (transient-reconnect semantics NOT weakened). Existing test_teardown.py `test_transport_disconnect_releases_after_reconnect_grace_elapses` + `test_reconnect_within_grace_cancels_teardown` still GREEN. Full server suite: 173 passed (172 + 1 net new), 0 regressions (quota, session lifecycle, reconnect-grace, idle-teardown, winddown all green). Live production verification (3+ consecutive reloads each releasing immediately, no ~30s wall) deferred to orchestrator post-deploy; deploys on top of the BUG 2/3 image currently building."

files_changed_refinement:
  - "apps/voice/server.py (_wire_connection_teardown: `closed` -> lifecycle.release() immediate, was on_transport_disconnected()/12s grace; docstring rewritten with the terminal-vs-transient rationale)"
  - "apps/voice/src/klanker_voice/session.py (start(): `if self._stopped: return` guard before tick/timer/watchdog creation — prevents the tick loop re-leasing on a released-mid-start interleave)"
  - "apps/voice/tests/test_slot_leak.py (abrupt test refined to assert IMMEDIATE release via a long grace + no sleep; added test_transport_disconnect_still_uses_the_reconnect_grace contrast lock)"
...
- checked (REFINEMENT cycle — terminal-vs-transient): pipecat connection.py `_handle_new_connection_state` (fires `closed` only when pc.connectionState=='closed', i.e. after pc.close()); the `failed` branch that self-closes; `renegotiate(restart_pc)` removing listeners before `_close()`; transport.py `on_disconnected`->`_handle_peer_disconnected` (does NOT reach on_client_disconnected) vs `on_closed`->`_handle_client_closed`->on_client_disconnected; request_handler.py `handle_disconnected` (`closed` handler pops _pcs_map, logs 'Discarding peer connection'); base_object.py `_call_event_handler` (async handlers dispatched as concurrent tasks tracked in _event_tasks).
  found: the connection `closed` event is TERMINAL-ONLY — it never fires on a transient blip (that fires `disconnected`) nor on restart_pc renegotiation (listeners removed first). After `closed` the pc is popped and a returning client gets a fresh session_id, so same-session reconnect is impossible; the reconnect grace on this path was waiting for a reconnect that can never come.
  implication: rev :13 bug confirmed — terminal close was routed through the 12s grace, stacking draining slots on rapid reloads and walling the 3rd. Safe fix = immediate release() on `closed`; keep the grace only for the transport's transient on_transport_disconnected path.
- checked: TDD RED (`test_abrupt_connection_close_releases_heartbeat_lease_immediately` with grace=30s + no sleep -> count==1) then GREEN after `_wire_connection_teardown` -> release() + the session.start() `_stopped` guard; new transient-contrast lock; full `.venv/bin/pytest -q`.
  found: RED reproduced the 30s-held slot; GREEN releases synchronously on `closed`; transient path still holds-through-grace; full suite 173 passed, 0 regressions.
  implication: BUG 1 refinement fixed + regression-locked (terminal immediate-release + transient-grace both locked side-by-side). Ready for deploy on top of the BUG 2/3 image.
