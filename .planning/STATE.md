...
last_updated: "2026-07-07T00:49:13.801Z"
...
Phase 05.2 (Slick Start — single-tap silent SSO + instant pre-rendered greeting): IN PROGRESS — 2/4 plans done. 05.2-01 (Workstream A, 05.2-01-SUMMARY.md): single-tap silent SSO — breadcrumb store, prompt=none attemptSilentSso guard, Callback login_required branch; 4/4 tasks, client tests + tsc build green; live verification deferred to the phase's consolidated pass. 05.2-04 (Workstream B, this plan, 05.2-04-SUMMARY.md): PersonaConfig.greet_first (default True, back-compat) read from `[persona]` TOML; pipeline.toml ships `greet_first = false`; server.py's `register_greet_first(...)` now guarded behind `cfg.persona.greet_first` so KPH doesn't greet twice on WebRTC now that the client plays a pre-rendered opener on tap; concierge.md "Opening move" rewritten so KPH doesn't re-introduce itself. 2 new config tests, full server suite 168/168 pass. Remaining: 05.2-02/03 (Workstream A/C client-side pre-rendered greeting clip + its wiring).
...
Last activity: 2026-07-06 — Mark voice deployed live (rev 10); CI deploy fixed (IAM applied + deploy.yml deadlock 260706-q42)
...
Last session: 2026-07-07T00:48:30.316Z
