...
resource "aws_kms_key" "ssm" {
  description              = "${var.site.label} DynamoDB SSM parameter encryption key - ${var.region.label}"
  deletion_window_in_days  = 30
  enable_key_rotation      = true
  policy                   = data.aws_iam_policy_document.ssm_key_policy.json

  tags = {
    Name      = "${var.site.label}-dynamodb-ssm-key-${var.region.label}"
    Site      = var.site.label
...
resource "aws_kms_alias" "ssm" {
  name          = "alias/${var.site.label}-dynamodb-ssm-${var.region.label}"
